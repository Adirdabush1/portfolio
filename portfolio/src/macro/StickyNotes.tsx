...
function makeNoteTexture(text: string, color: string): CanvasTexture {
  const w = 512;
  const h = 512;
...
function Note({ position, rotationY, color, text }: StickyProps) {
...
export default function StickyNotes({ notes }: { notes: StickyProps[] }) {
  return (
    <>
      {notes.map((n, i) => (
        <Note key={i} {...n} />
