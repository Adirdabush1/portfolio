...
const Body = () => (
  <group scale={[TOY.SCALE, TOY.SCALE, TOY.SCALE]}>
...
const ToyCar = forwardRef<ToyCarHandle>((_, ref) => {
  ...
  useFrame((_state, dt) => {
    ...
    if (Math.abs(turnAxis) > 0.05) {
      ...
      body.applyTorqueImpulse(
        { x: 0, y: TOY.TURN_RATE * turnAxis * dt * speedFactor * body.mass(), z: 0 },
        true,
      );
    }
    ...
    if (t.y < TOY.SPAWN[1] + 0.3) groundedRef.current = true;
    ...
    if (vy > TOY.MAX_VERTICAL_SPEED) vy = TOY.MAX_VERTICAL_SPEED;
  ...
  return (
    <RigidBody
      ...
      mass={TOY.MASS}
      position={TOY.SPAWN}
      linearDamping={TOY.LINEAR_DAMPING}
      angularDamping={TOY.ANGULAR_DAMPING}
