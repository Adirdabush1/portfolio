...
const Body = () => (_jsx(Suspense, { fallback: _jsx(CarFallback, {}), children: _jsx(CarModel, {}) }));
const Car = forwardRef((_, ref) => {
    ...
    useFrame((_state, dt) => {
        ...
        if (Math.abs(turnAxis) > 0.05) {
            ...
            body.applyTorqueImpulse({ x: 0, y: CAR.TURN_RATE * turnAxis * dt * speedFactor * body.mass(), z: 0 }, true);
        }
        ...
        if (vy > CAR.MAX_VERTICAL_SPEED)
            vy = CAR.MAX_VERTICAL_SPEED;
    ...
    return (_jsx(RigidBody, { ref: bodyRef, colliders: "cuboid", mass: CAR.MASS, position: CAR.SPAWN, linearDamping: CAR.LINEAR_DAMPING, angularDamping: CAR.ANGULAR_DAMPING, enabledRotations: [false, true, false], children: _jsx(Body, {}) }));
});
Car.displayName = "Car";
export default Car;
