...
export class DataStream {
  ...
  update(progress: number, time: number, scrollVelocity = 0) {
    ...
    (this.particles.material as THREE.MeshBasicMaterial).opacity = 0.5 * opacity;
    ...
    for (let i = 0; i < this.depthLines.length; i++) {
      ...
      (this.depthLines[i].material as THREE.LineBasicMaterial).opacity = 0.06 * opacity;
    }
  }
