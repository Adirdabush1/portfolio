...
export class AiCore {
  ...
  private orbitingNodes: THREE.Mesh[] = [];
  ...
  update(progress: number, time: number) {
    ...
    (this.core.material as THREE.MeshPhongMaterial).opacity = opacity * 0.8;
    (this.wireOverlay.material as THREE.LineBasicMaterial).opacity = opacity * 0.5;
    ...
    for (let i = 0; i < this.orbitingNodes.length; i++) {
      ...
      (this.orbitingNodes[i].material as THREE.MeshPhongMaterial).opacity = nodeAppear * (1 - fadeOut);

      // Update connection line
      const linePos = this.orbitLines[i].geometry.getAttribute("position") as THREE.BufferAttribute;
      linePos.setXYZ(0, 0, 0, 0);
      linePos.setXYZ(1, x, y, z);
      linePos.needsUpdate = true;
      (this.orbitLines[i].material as THREE.LineBasicMaterial).opacity = nodeAppear * 0.15 * (1 - fadeOut);
    }

    // Point light
    const light = this.group.children.find((c) => c instanceof THREE.PointLight) as THREE.PointLight;
    if (light) light.intensity = 1.5 * opacity;
    ...
    this.particles.instanceMatrix.needsUpdate = true;
    (this.particles.material as THREE.MeshBasicMaterial).opacity = 0.2 * opacity;
  }
