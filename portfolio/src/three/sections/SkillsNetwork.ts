...
export class SkillsNetwork {
  ...
  private ambientParticles: THREE.InstancedMesh;
  private ambientCount: number;
  private ambientBasePos: Float32Array;
  private dummy = new THREE.Object3D();

  constructor(scene: THREE.Scene) {
    this.targetPositions = skillNodes.map((s) => new THREE.Vector3(s.pos[0], s.pos[1], s.pos[2]));
    this.startPositions = skillNodes.map(() => new THREE.Vector3(
      (Math.random() - 0.5) * 0.5,
      (Math.random() - 0.5) * 0.5,
      (Math.random() - 0.5) * 0.5
    ));
  ...
  update(progress: number, time: number) {
    ...
    for (let i = 0; i < this.nodes.length; i++) {
      ...
      (node.material as THREE.MeshPhongMaterial).opacity = opacity;
    }

    // Update connection lines
    const lineOpacity = THREE.MathUtils.clamp((spreadProgress - 0.5) * 2, 0, 0.35) * (1 - fadeOut);
    (this.lineSegments.material as THREE.LineBasicMaterial).opacity = lineOpacity;
    const posArr = this.linePositions;
    for (let i = 0; i < connections.length; i++) {
      const [a, b] = connections[i];
      const na = this.nodes[a].position;
      const nb = this.nodes[b].position;
      posArr[i * 6] = na.x; posArr[i * 6 + 1] = na.y; posArr[i * 6 + 2] = na.z;
      posArr[i * 6 + 3] = nb.x; posArr[i * 6 + 4] = nb.y; posArr[i * 6 + 5] = nb.z;
    }
    ...
    for (let i = 0; i < this.ambientCount; i++) {
      ...
      this.dummy.position.set(
        bx + Math.sin(time * 0.3 + i * 0.2) * 0.1,
        by + Math.cos(time * 0.25 + i * 0.15) * 0.08,
        bz
      );
      this.dummy.updateMatrix();
      this.ambientParticles.setMatrixAt(i, this.dummy.matrix);
    ...
    this.ambientParticles.instanceMatrix.needsUpdate = true;
    (this.ambientParticles.material as THREE.MeshBasicMaterial).opacity = 0.3 * opacity;
  }
