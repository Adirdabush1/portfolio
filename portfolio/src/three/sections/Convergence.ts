...
export class Convergence {
  ...
  private ringParticles: THREE.InstancedMesh;
  private convergingParticles: THREE.InstancedMesh;
  private ringCount: number;
  private convergeCount: number;
  ...
  update(progress: number, time: number) {
    ...
    for (let i = 0; i < this.convergeCount; i++) {
      ...
      this.dummy.position.set(
        r * Math.sin(phi) * Math.cos(theta + time * 0.2),
        r * Math.sin(phi) * Math.sin(theta + time * 0.15),
        r * Math.cos(phi)
      );
      this.dummy.updateMatrix();
      this.convergingParticles.setMatrixAt(i, this.dummy.matrix);
    ...
    this.convergingParticles.instanceMatrix.needsUpdate = true;
    (this.convergingParticles.material as THREE.MeshBasicMaterial).opacity =
      0.4 * (1 - orbAppear * 0.5);

    // Orb appears after particles converge
    (this.orb.material as THREE.MeshPhongMaterial).opacity = orbAppear * 0.7;
    ...
    this.ringParticles.instanceMatrix.needsUpdate = true;
    (this.ringParticles.material as THREE.MeshBasicMaterial).opacity = orbAppear * 0.5;
  }
