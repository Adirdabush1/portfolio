...
export class HeroBrain {
    constructor(scene) {
        ...
        for (let i = 0; i < this.particleCount && lineCount < maxLines; i += 10) {
            for (let j = i + 1; j < this.particleCount && lineCount < maxLines; j += 10) {
                ...
                if (Math.sqrt(dx * dx + dy * dy + dz * dz) < 2.5) {
                    linePositions.push(this.particleBasePos[i * 3], this.particleBasePos[i * 3 + 1], this.particleBasePos[i * 3 + 2], this.particleBasePos[j * 3], this.particleBasePos[j * 3 + 1], this.particleBasePos[j * 3 + 2]);
                    lineCount++;
                }
            }
        }
        ...
        for (let i = 0; i < this.tunnelCount; i++) {
            ...
            const c = new THREE.Color().lerpColors(new THREE.Color(0x4a90d9), new THREE.Color(0xefd09e), depthT * 0.5);
        ...
        this.group.add(this.outerSphere, this.innerSphere, this.wireframe, this.particles, this.lines, this.tunnelParticles, pointLight, innerLight);
    ...
    update(progress, time) {
        ...
        this.outerSphere.material.opacity = 0.15 * (1 - enterProgress * 0.7);
        this.wireframe.material.opacity = 0.35 * (1 - enterProgress * 0.5);
        // Inner sphere glows brighter as we approach then fades as we pass through
        const innerGlow = Math.sin(enterProgress * Math.PI);
        this.innerSphere.material.opacity = 0.2 + innerGlow * 0.4;
        this.innerSphere.material.emissiveIntensity = 0.5 + innerGlow * 1.5;
        // Orbiting particles
        for (let i = 0; i < this.particleCount; i++) {
            ...
            this.dummy.position.set(bx * cos - bz * sin * 0.1 + Math.sin(time * 0.5 + i * 0.1) * 0.15, by + Math.cos(time * 0.3 + i * 0.15) * 0.1, bz * cos + bx * sin * 0.1);
            this.dummy.updateMatrix();
            this.particles.setMatrixAt(i, this.dummy.matrix);
        }
        this.particles.instanceMatrix.needsUpdate = true;
        ...
        for (let i = 0; i < this.tunnelCount; i++) {
            ...
            this.dummy.position.set(bx + Math.sin(swirl + i * 0.05) * 0.15, by + Math.cos(swirl + i * 0.07) * 0.12, bz);
            this.dummy.updateMatrix();
            this.tunnelParticles.setMatrixAt(i, this.dummy.matrix);
        }
        this.tunnelParticles.instanceMatrix.needsUpdate = true;
